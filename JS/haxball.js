/**
 * 
 * Gab's HaxMod Alpha Version by gabius
 * 
 * This is a modification of Haxball's game source
 * 
 * Use the /modhelp command to get the list of available features
 * 
 */

(function (qc) {
    const DEFAULT_SHAKE_VALUE = 4;
    ...
    let showHackInfo = true;
    let keysPressing = { keys: [], persistance: 0 };
    ...
    const HackMenuSections = {
        Mods: { name: "mods-hsec", label: "Mods" },
        Input: { name: "input-hsec", label: "Input" },
        Values: { name: "values-hsec", label: "Values" },
        Actions: { name: "actions-hsec", label: "Actions" },
    ...
    const HackOptions = {
        ...
        OpMode: {
            ...
            input: { configurable: true, keyCode: "ControlLeft" },
            state: { closeToBall: false }
        },
        Macro: {
            name: "macro",
            label: "Macro",
            enabled: true,
            input: { configurable: true, keyCode: "KeyC" },
            value: 0,
            state: { running: false, click: false }
        },
        AutoShake: {
            name: "autoshake",
            label: "AutoShake",
            enabled: false,
            input: { configurable: true, keyCode: "KeyG" },
            value: DEFAULT_SHAKE_VALUE,
            state: { wait: 0, keyIndex: 0 }
        },
        AutoKick: {
            name: "autokick",
            label: "AutoKick",
            enabled: false,
            input: { configurable: true, keyCode: "KeyF" },
            state: { /** false, "up", "down" */ kickNext: false }
        },
        ChatBubble: {
            name: "chatbubble",
            label: "Chat Bubble",
            ...
            value: 0,
            input: { configurable: false },
            state: { bubbleNext: false, interval: null }
        },
        FakePing: {
            name: "fakeping",
            label: "Fake Ping",
    ...
    function getHackMenuSections() {
        return Object.values(HackMenuSections).filter(h => h?.name != null);
    }

    function setChatBubble(value) {
        roomObj?.ra(na.la(!value));
    }

    function setAvatar(value) {
        roomObj?.ra(ra.la(value));
    }
    ...
    function getHackViewHtml() {
        ...
        <div data-hook="hack-menu" data-edit-hash="$HASH_HERE" style='display: flex; flex-direction: column; align-items: center; justify-content: center; position: absolute; top: 50%; right: 50%; transform: translate(50%,-50%); height: 350px; width: 350px; border: 5px solid rgba(0,0,0,.005); color: rgb(255, 255, 225); background-color: rgba(0,0,0,.5); font-weight: bold; font-size: 1.2em;'>
            ...
            <p style='position: absolute; top: 10%; color: orange; text-align: center; font-size: 1.6em;'>Gab's HaxMod</p>

            <div class='hack-menu-tabs' style='position: absolute; top: 22%;'>
                ${getHackMenuSections().map(h => `<button style="padding: 5 20px; color: white; background-color: #244967; width: 70px; cursor: pointer; border: 0;" data-hook='${h.name}'>${h.label}</button>`).join("")}
            </div>

            <div class='hack-menu-body' style='display: flex; flex-direction: column; position: absolute; top: 40%;'>
                ${
                    (() => {
                        ...
                        switch (HackMenuSections.currentSection) {
                            case HackMenuSections.Mods.name:
                                return hackOptionsList.map(h => `<div data-hook='${h.name}-opt' style="cursor: pointer; padding-bottom: 3px;"><i class='icon-${h.enabled ? "ok" : "cancel"}'></i> ${h.label}</div>`);
                            ...
                            case HackMenuSections.Values.name:
                                return hackOptionsList.filter(h => h.value != null && h.name !== HackOptions.ChatBubble.name)
                                    .concat({ ...HackOptions.ChatBubble, label: "Chat Bubble Blink" })
                                    .map(h => `<div style="padding-bottom: 3px; display: flex; justify-content: space-between; width: 250px; max-width: 250px;">${h.label}<input onfocusout="saveValue='${h.name}';" data-hook='${h.name}-val' style="color:black; background-color: white; width: 100px; border: 0; text-align: center;" value="${h.value}"></input></div>`)
    ...
    function getStatsViewElementString() {
        ...
        <div data-hook="hack-info" data-edit-hash="$HASH_HERE" style='position: fixed; left: 10px; top: 10px; border: 5px solid rgba(0,0,0,.005); color: rgb(255, 255, 225); background-color: rgba(0,0,0,.1); font-weight: bold; font-size: 0.8em;'>
            <p style='color: orange; text-align: center;'>Gab's HaxMod</p>
            <p>Aimbot: ${HackOptions.AimBot.enabled ? "Enabled" : "Disabled"}</p>
            <p>OP Mode: ${HackOptions.OpMode.enabled ? "Enabled" : "Disabled"}</p>
            <p>Fake ping: ${HackOptions.FakePing.enabled ? HackOptions.FakePing.value : "Disabled"}</p>
            <p>Chat bubble: ${HackOptions.ChatBubble.enabled ? "Enabled" : "Disabled"}</p>
            <p>Macro: ${HackOptions.Macro.state.running ? "On" : "Off"}</p>
            <p>Autokick: ${HackOptions.AutoKick.enabled ? "Enabled" : "Disabled"}
            <p>AutoShake: ${HackOptions.AutoShake.enabled ? "Enabled" : "Disabled"}
            <p>AutoShake value: ${HackOptions.AutoShake.value}
            <p>OP Mode value: ${HackOptions.OpMode.value}</p>
            ${qc?.ips ? qc.ips?.map(a => `<p style='color: lightblue;' onclick='navigator.clipboard.writeText("${a}");'>${a}</p>`).join("") : ""}
        </div>`;
    ...
    function avatarCommand(args, send) {
        ...
        avatarInterval = setInterval(() => {
            if (playerPosition && avatarList) {
                if (++avatarIndex > avatarList?.length - 1) avatarIndex = 0;
    ...
    function helpCommand(_, sendInfo) {
        for (const line of [
            "How to use - Gab's HaxMod",
            "- Toggle, change values, keys and access mod features in the menu option.",
            "- New alternative avatar command syntax for special avatars. Usage: /avatar <time> <avatar1,avatar2>",
            "- /ban_all - Bans everyone from the room.",
            "- /toggle_stats - Toggle mod side view."
        ]) sendInfo(line);
    }
    ...
    Ra.prototype = {
        ...
        C: function () {
            ...
            keysPressing = {
                keys: this.$d,
                persistance: this.$d === keysPressing.keys ? (keysPressing.persistance ?? 0) + 1 : 0
            };
    ...
    N.prototype = {
        ...
        Kc: function (a, b) {
            ...
            if (null != a.K) {
                ...
                playerPosition = player?.H?.a ? { name: player.w, ...player.H.a, radius: player.H.Z } : null;

                /* Register ball position */

                const ball = e.F[0];
                const ballRadius = ball.Z;
                ballPosition = ball?.a ? { ...ball?.a, radius: ballRadius } : null;
                ...
                const playerDistanceMap = a.I
                    .filter(p => p.H?.a)
                    .map(p => {
                        return { distance: getDistance({ name: p.w, ...p.H.a, radius: p.H.Z }, ballPosition), x: p.H.a.x, y: p.H.a.y, team: p.ea.w };
                    })
                ...
                if (HackOptions.AimBot.enabled && closestPlayerPosition?.distance < MIN_DISTANCE) {
    ...
    Xa.prototype = {
        qr: function (a) {
            this.rg.textContent = null == a ? "null" : a//getFakePingInfo(a)
        },
        or: function (a) {
    ...
    Da.N = "<div class='chatbox-view'><div data-hook='log' class='log'><p>Gab's HaxMod v1 by gabius</p></div><div class='autocompletebox' data-hook='autocompletebox'></div><div class='input'><input data-hook='input' type='text' /><button data-hook='send'>Send</button></div></div>";
